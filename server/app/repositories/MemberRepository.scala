...
import services.{DefaultBCryptService, DefaultJwtService}
...
class MongoMemberRepository @Inject() (
  ...
  def createOne(newMember: Member): Future[Option[Member]] = {
    val insertedMember = Member(newMember.username, DefaultBCryptService.cryptPassword(newMember.password))
  ...
  def updateOne (username: String, newMember: MemberUpdateRequest): Future[Option[Unit]] = {
    val updatedMember = Member(username, DefaultBCryptService.cryptPassword(newMember.newPassword))
  ...
  def auth (memberAuth: Member): Future[Option[String]] = {
    findByUsername(memberAuth.username).map {
      case Some(member) if DefaultBCryptService.checkPassword(memberAuth.password, member.password)
        => Some(DefaultJwtService.generateToken(member.username))
      case _ => None
    }
  }
  ...
  def update (username: String, tokenUsername: String, memberUpdateRequest: MemberUpdateRequest): Future[Either[Exception, Unit]] = {
    findByUsername(username)
      .flatMap {
        case Some(member) if checkUserPermissions(username, tokenUsername) && DefaultBCryptService.checkPassword(memberUpdateRequest.password, member.password) =>
